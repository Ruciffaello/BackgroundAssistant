...
專案根目錄只保留總覽與版本歷史；工作管理、設計、研究和測試文件集中在 `docs/`。

## 專案管理

| 文件 | 用途 |
| --- | --- |
| [專案總覽](../README.md) | 現況、架構與文件入口 |
...
- 根目錄：只保留新進開發者需要立即看到的 `README.md` 與 `CHANGELOG.md`。
- `docs/project/`：工作清單、決策與交接紀錄。
- `docs/design/`：目前採用的架構與設計規格。
- `docs/research/`：尚未採用或需要重新評估的研究與提案。
- `docs/testing/`：可重複執行的測試方式、劇本與驗收清單。
